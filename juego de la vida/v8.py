...
def main():
    ...
    # Control de la velocidad de evolución
    simulation_delay = 0.2  # segundos entre actualizaciones
    sim_timer = 0

    while running:
        dt = clock.tick(FPS) / 1000.0  # tiempo transcurrido en segundos
        sim_timer += dt
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE:
                    if len(positions) == 0:
                        print("⚠ No hay células vivas. Presiona G para generar o dibuja alguna.")
                    else:
                        playing = not playing
                    ...
                    iteration = 0
                    history.clear()
                    viewport_x = max(0, min(GRID_WIDTH - WIDTH // cell_size, GRID_WIDTH // 2 - WIDTH // (2 * cell_size)))
                    viewport_y = max(0, min(GRID_HEIGHT - HEIGHT // cell_size, GRID_HEIGHT // 2 - HEIGHT // (2 * cell_size)))

        # Actualizamos la simulación solo si ha pasado el tiempo indicado
        if (playing or step_mode) and sim_timer >= simulation_delay:
            sim_timer = 0  # reiniciamos el temporizador
            if len(positions) == 0:
                playing = False
